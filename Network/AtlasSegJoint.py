...
class AttentionJoint(nn.Module):
    def __init__(self, in_chan, out_chan):
        ...
        self.atlas_conv = MixConv(in_chan // 4, out_chan // 4)
...
class AtlasSegmentor(nn.Module):
    def __init__(self):
        ...
        self.out = out_stage(self.jointchannels)
...
class AtlasSegJointNet(nn.Module):
    def __init__(self):
        super(AtlasSegJointNet, self).__init__()
...
if __name__ == '__main__':
    net = AtlasSegJointNet().cuda()
    torch.save(net.state_dict(), "AtlasSegJointNet.pth.gz")
