            ...
            train_dict_list.append(
                {
                    'image_path': os.path.join(path, image_name),
                    'label_path': os.path.join(path, label_name),
                    'GA_path': os.path.join(path, GA_name),
                }
            )
        for path in test_path_list:
            test_dict_list.append(
...
class TrainGenerator(object):
    ...
    def get_item(self):
        ...
        elif self.net_type[:2] == 'GA':
            GA_list = [dict_item['GA_path'] for dict_item in dict_list]
            image_patch, label_patch, GA_patch = self._sample_patch(image_list, label_list, GA_list=GA_list)
            return image_patch, label_patch, GA_patch
        else:
    ...
    def _sample_patch(self, image_list, clean_list, atlas_image_list=None, atlas_label_list=None, GA_list=None):
        ...
        if self.net_type[:8] == 'AtlasSeg':
            ...
            for image_path, clean_path, atlas_image_path, atlas_label_path in zip(image_list, clean_list,
                ...
                image, label, atlas_image, atlas_label = RandomContrast(alpha=(0.9, 1.1), execution_probability=0.15)(
                    image, label, atlas_image, atlas_label)
                image, label, atlas_image, atlas_label = ElasticDeformation(alpha=(-1000, 1000),
                                                                            execution_probability=0.15)(image, label,
                                                                                                       atlas_image,
                                                                                                       atlas_label)
            ...
            GA_patch_list = []
            for image_path, clean_path, GA_path in zip(image_list, clean_list, GA_list):
                image = nib.load(image_path).get_fdata()
                label = nib.load(clean_path).get_fdata()
                ...
                image_patch = image.astype(np.float32)
                label_patch = label.astype(np.float32)
                GA_patch = np.reshape(np.array(float(GA)), [1]).astype(np.float32)

                image_patch_list.append(image_patch[np.newaxis, np.newaxis, ...])
                label_patch_list.append(label_patch[np.newaxis, np.newaxis, ...])
                GA_patch_list.append(GA_patch[np.newaxis, :])

            image_out = np.concatenate(image_patch_list, axis=0)
            label_out = np.concatenate(label_patch_list, axis=0)
            GA_out = np.concatenate(GA_patch_list, axis=0)

            return image_out, label_out, GA_out
